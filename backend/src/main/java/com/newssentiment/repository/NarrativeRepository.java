...
public interface NarrativeRepository extends JpaRepository<Narrative, Long> {
    ...
    /**
     * Find all auto-generated narratives that are not PENDING_REVIEW.
     * Used by admin endpoint to reset them for re-review.
     */
    @Query("SELECT n FROM Narrative n WHERE n.autoGenerated = true AND n.status != 'PENDING_REVIEW'")
    List<Narrative> findAutoGeneratedNotPendingReview();

    /**
     * Count auto-generated narratives by status.
     */
    @Query("SELECT n.status, COUNT(n) FROM Narrative n WHERE n.autoGenerated = true GROUP BY n.status")
    List<Object[]> countAutoGeneratedByStatus();
}
